...
namespace AppWindow {
	...
	std::string wstring_to_string(std::wstring wideString);
	std::string open_file_dialog(COMDLG_FILTERSPEC ComDlgFS[], int size);	
	// < Functions

	// TODO: Zabezpieczenia + wyj¹tki
	class Window {
	private:
		static HWND resized_wnd_handle_;
		static int resized_wnd_width_;
		static int resized_wnd_height_;
		std::string title_;
		HWND wnd_handle_;
