...
int main(int argc, const char** argv) {
	...
	try {
		...
		LightEngine::LightViewCamera light_camera(core);
		...
		light_camera.set_orthographic_projection();
		...
		camera_space_z = light_camera.get_z_axis();
		...
		while (WM_QUIT != msg.message) {
			...
			if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
				...
				// GUI Scope
				{
					...
					if(ImGui::InputFloat3("Position", lc_position,"%.2f")) {
						light_camera.set_position(lc_position[0], lc_position[1], lc_position[2]);
						light_camera.update_view_matrix();
						update_camera = true;
					}

					if(ImGui::SliderInt2("Angle V/H", lc_angles, -360.0, 360.0)) {
						light_camera.set_vertical_angle(lc_angles[0]);
						light_camera.set_horizontal_angle(lc_angles[1]);
						light_camera.update_view_matrix();
						direct_light_position[0] = camera_space_z[0];
						direct_light_position[1] = camera_space_z[1];
						direct_light_position[2] = camera_space_z[2];
						direct_light.set_position(direct_light_position);
						update_light = true;
						update_camera = true;
					...
					if (ImGui::SliderInt("Scaling", &lc_scaling, 1, 200)) {
						light_camera.set_orthographic_scaling(lc_scaling);
						light_camera.update_projection_matrix();
						update_camera = true;
					}
					...
					if (ImGui::BeginMenu("Direct light")) {
						...
						if (ImGui::DragFloat("D. Intensity", &direct_light_intensity, 10.0f, 1.0f, 1000.0f, "%.2f")) {
				...
				if (update_camera) {
					...
					light_camera.update();
					update_camera = false;
					std::cout<<camera_space_z[0]<<","<<camera_space_z[1]<<","<<camera_space_z[2]<<std::endl;
				}
